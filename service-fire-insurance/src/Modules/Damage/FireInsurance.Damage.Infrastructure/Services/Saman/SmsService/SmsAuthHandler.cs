        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrEmpty(token))
            {
                ...
                if (isOnStage)
                {
                    var stageResponse = await stageAuthApi.GetTokenAsync(tokenRequest.UserName, tokenRequest.Password);
                    response = new TokenResponse
                    {
                        Result = stageResponse.Token,
                        Code = stageResponse != null ? 200 : 401,
        ...
        try
        {
            ...
            if (httpResponse.StatusCode is HttpStatusCode.Unauthorized or HttpStatusCode.NotAcceptable)
            {
